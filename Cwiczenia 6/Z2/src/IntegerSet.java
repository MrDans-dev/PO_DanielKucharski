    ...
    public static IntegerSet union(IntegerSet set1, IntegerSet set2){
        IntegerSet set = new IntegerSet();
         for(int i = 0; i<=set1.set.length-1; i++){
             if(set1.set[i] == true  || set2.set[i] == true) set.set[i] = true;
             if(set1.set[i] == false && set2.set[i] == false) set.set[i] = false;
            }
     ...
     public static IntegerSet intersection(IntegerSet set1,IntegerSet set2){
         IntegerSet set = new IntegerSet();
         for(int i = 0; i<=set1.set.length-1; i++){
             if(set1.set[i] == set2.set[i]) set.set[i] = set1.set[i];
             if(set1.set[i] != set2.set[i]) set.set[i] = false;
            }
         return set;
     }
